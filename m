...
CC = "gcc"
DEFAULT_CXFLAGS = ("-Wall", "-Wextra", "-std=c17", "-O2", f"-I{rel(SOURCE, 'test')}", "-c")
DEFAULT_LDFLAGS = ()
...
def ld(target: str, files: list[str], *, addition_ldflags: tuple[str] = ()):
...
# type: ignore
def cc(src_path: str, obj_path: str, *, addition_cxflags: tuple[str] = (), addition_ldflags: tuple[str] = ()):
...
def mpicc(src_path: str, obj_path: str):
    pass
...
# type: ignore
def simple_build(target: str, *, addition_cxflags: tuple[str] = (), addition_ldflags: tuple[str] = ()):
    build_test()
...
config = {
    ...
    "03_omp": lambda: simple_build("03_omp", addition_cxflags=("-fopenmp",))
}
